...
public class SearchOperationsIntegrationTest extends AbstractIntegrationTest {
    ...
    public void testBasicSearchByTermOnly() {
        final SearchConfig cfg = searchOps.builder().q(FILE_SEARCH_TERM).build();
        final DataverseResponse<SearchResults<Item>> results = searchOps.search(cfg);
        assertNotNull(results.getMessage(), results.getData());
    }

    @Test
    public void testComplexSearch() {
        final SearchConfig cfg = searchOps.builder().q(FILE_SEARCH_TERM)
                .sortBy(SortBy.date)
